...
def main():
    ...
    acceptance_tmp = Path(tempfile.gettempdir()) / 'aurum_rc3_pytest_acceptance'
    test_files = [
    ...
    run(py,'-m','pytest','--basetemp',str(acceptance_tmp),*test_files,'-q',env=env)
    ...
    if v2.exists():
        ...
        run(py,'-m','pytest','--basetemp',str(acceptance_tmp.with_name('aurum_v2_rc3_pytest_acceptance')),'tests/test_mars_cvar_engine_bridge.py','-q',cwd=v2,env=env2)
